...
data <- read.csv('/Users/alvinyao/Desktop/UChicago/Academics/2 - Winter 2025/31006 - TS Analysis & Forecasting/Final Project/adsp31006-time-series-analysis/data/avg_sst_california.csv')
...
best_sarima <- auto.arima(train_transformed, seasonal=TRUE, lambda = lambda, approximation = FALSE, stepwise = FALSE)
...
best_arfima <- arfima(train_transformed, 
                      order = c(1, 0, 0), 
                      lmodel = list(dfrac = d_adjusted))
...
sarima_cv <- function(y, h) {
  forecast(
    Arima(y, order = c(1,0,1), seasonal = list(order=c(2,1,0), p=12)),
    h = h)
}

cv_errors <- tsCV(train_transformed, sarima_cv, h = 1)
