...
```
```{r}
manual_sarima <- Arima(train_data, lambda=lambda, order=c(2,0,1), seasonal=c(5,1,0))
manual_sarima

checkresiduals(manual_sarima)

acf(residuals(manual_sarima), lag.max = 96)
```
...
Box.test(residuals(manual_sarima), lag=48, type="Ljung-Box")
```
